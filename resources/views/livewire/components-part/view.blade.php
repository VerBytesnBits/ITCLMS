<div x-data x-on:keydown.escape.window="$dispatch('closeModal')"
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 flex justify-between items-center">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Component Details</h2>
        ...
        <div class="px-6 py-4 space-y-4">
            <div class="grid grid-cols-2 gap-4 text-sm">
                {{-- <div><span class="font-medium">ID:</span> {{ $component->id }}</div> --}}
                <div><span class="font-medium">Unit:</span> {{ optional($component->systemUnit)->name ?? '—' }}</div>
                <div><span class="font-medium">Serial Number:</span> {{ $component->serial_number }}</div>
                <div><span class="font-medium">Brand:</span>{{ ucwords(strtolower($component->brand)) }}</div>
                ...
                <div class="col-span-2"><span
                        class="font-medium">Warranty:</span>{{ $component->warranty ? \Carbon\Carbon::parse($component->warranty)->format('M d, Y') : '—' }}
                </div>
                {{-- <div class="col-span-2"><span class="font-medium">Room:</span> {{ optional($component->room)->name ?? '—' }}</div> --}}
            </div>
        </div>
