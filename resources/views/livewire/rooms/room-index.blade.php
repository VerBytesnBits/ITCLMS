<div class="space-y-6">
    ...
    <div class="flex justify-between max-sm:flex-col">
        <div>
            <flux:heading size="xl" level="1" class="text-2xl! font-bold! text-zinc-500 dark:text-zinc-50 flex items-center gap-2 leading-tight italic"> Lab. Rooms</flux:heading>
            <flux:subheading size="lg" class="italic">Manage all laboratory rooms</flux:subheading>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <!-- Room Card -->
        <div class="relative bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 
           rounded-2xl shadow-md hover:shadow-xl transition transform hover:-translate-y-1 
           flex flex-col overflow-hidden">

            <!-- Header -->
            <div class="p-5 border-b border-zinc-200 dark:border-zinc-700 flex justify-between items-start bg-gradient-to-r from-green-500 to-emerald-600">
                ...
                @endphp
                <span class="px-3 py-1 rounded-full text-xs font-medium {{ $statusColors[$room->status] ?? 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300' }}">
                    {{ $room->status ?? '—' }}
                </span>
            </div>
            ...
            <div class="p-5 flex-1 space-y-5">
                ...
                <div class="flex flex-col gap-2">
                    <div class="flex items-center justify-between">
                        ...
                        @role('chairman')
                        <button wire:click="openAssignLabIncharge({{ $room->id }})" class="group relative flex items-center justify-center w-7 h-7 rounded-full 
                       bg-purple-500 hover:bg-purple-600 text-white transition">
                            <flux:icon.circle-plus class="w-4 h-4" />

                            <!-- Tooltip -->
                            <span class="absolute right-9 top-1/2 -translate-y-1/2 whitespace-nowrap
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <span class="flex items-center gap-2 px-3 py-1 rounded-lg text-xs font-medium
                            ...
                            <button wire:click="removeIncharge({{ $room->id }}, {{ $incharge->id }})" class="group-hover:opacity-100 transition-opacity duration-200
                                        text-gray-500 hover:text-red-700 dark:hover:text-red-400" title="Unassign Lab In-Charge">
                ...
                <div class="flex flex-col gap-2">
                    <div class="flex items-center justify-between">
                        ...
                        <button wire:click="openAssignTechnician({{ $room->id }})" class="group relative flex items-center justify-center w-7 h-7 rounded-full 
           ...
           rounded-b-2xl">
                ...
                <div class="flex items-center gap-2">
                    <flux:button wire:click="openEditModal({{ $room->id }})" icon="pencil" variant="primary" size="sm" class="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 
           ...
           font-medium rounded-base text-sm px-4 py-2.5 inline-flex items-center gap-1">
                    ...
                    <flux:button wire:click.prevent="confirmDeleteRoom({{ $room->id }})" variant="primary" icon="trash" size="sm" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 
        ...
        <div class="col-span-full flex flex-col items-center justify-center py-16 text-center">
            ...
            <flux:button variant="primary" color="green" wire:click="openCreateModal" class="px-5 py-2 rounded-full shadow-md hover:shadow-lg transition">
                + Add Room
            </flux:button>
        </div>
        @endforelse
