<div class="p-4 space-y-6">
    ...
    <div class="relative w-full px-4 py-2">
        <flux:heading size="xl" level="1">{{ __('CPUs') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all CPU records') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <!-- Add Button -->
    <div class="flex justify-start">
        <button wire:click="$dispatch('open-cpu-form')"
            ...
            <tbody>
                @foreach ($items as $cpu)
                    <tr
                        ...
                        <td class="px-6 py-4">{{ $cpu->systemUnit->name ?? '' }}</td>
                        <td class="px-6 py-4">{{ $cpu->serial_number }}</td>
                        <td class="px-6 py-4">{{ $cpu->brand }}</td>
                        <td class="px-6 py-4">{{ $cpu->model }}</td>
                        <td class="px-6 py-4">{{ $cpu->speed }}</td>
                        <td class="px-6 py-4">{{ $cpu->condition }}</td>
                        <td class="px-6 py-4">{{ $cpu->status }}</td>
                        <td class="px-6 py-4">{{ $cpu->warranty }}</td>

                        <td class="px-6 py-4 text-right space-x-3">
                            <button wire:click="edit({{ $cpu->id }})"
