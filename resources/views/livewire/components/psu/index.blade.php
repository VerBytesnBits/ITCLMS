<div class="p-4 space-y-6">
    ...
    <div class="relative w-full px-4 py-2">
        <flux:heading size="xl" level="1">{{ __('PSUs') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all power supply units') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <!-- Add Button -->
    <div class="flex justify-start">
        <button wire:click="$dispatch('open-psu-form')"
    ...
    <div
        ...
        <table class="min-w-full text-sm text-left text-gray-700 dark:text-gray-200">
            ...
            <tbody>
                @foreach ($items as $psu)
                    <tr
                        class="border-t border-gray-200 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                        <td class="px-6 py-4 font-medium text-zinc-800 dark:text-white">{{ $psu->id }}</td>
                        <td class="px-6 py-4">{{ $psu->systemUnit->name ?? '' }}</td>
                        <td class="px-6 py-4">{{ $psu->serial_number }}</td>
                        <td class="px-6 py-4">{{ $psu->brand }}</td>
                        <td class="px-6 py-4">{{ $psu->model }}</td>
                        <td class="px-6 py-4">{{ $psu->wattage }}</td>
                        <td class="px-6 py-4">{{ $psu->efficiency }}</td>
                        <td class="px-6 py-4">{{ $psu->condition }}</td>
                        <td class="px-6 py-4">{{ $psu->status }}</td>
                        <td class="px-6 py-4">{{ $psu->warranty }}</td>
                        <td class="px-6 py-4 text-right space-x-3">
                            <button wire:click="edit({{ $psu->id }})"
                                class="text-blue-500 text-sm font-medium hover:underline cursor-pointer">Edit</button>
                            <button wire:click="delete({{ $psu->id }})"
