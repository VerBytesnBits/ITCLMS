<div class="p-4 space-y-6">
    ...
    <div class="relative w-full px-4 py-2">
        <flux:heading size="xl" level="1">{{ __('Drives') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all storage drives') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <!-- Add Button -->
    <div class="flex justify-start">
        <button wire:click="$dispatch('open-drive-form')"
            ...
            <tbody>
                @foreach ($items as $drive)
                    <tr
                        ...
                        <td class="px-6 py-4">{{ $drive->systemUnit->name ?? '' }}</td>
                        <td class="px-6 py-4">{{ $drive->serial_number }}</td>
                        <td class="px-6 py-4">{{ $drive->brand }}</td>
                        <td class="px-6 py-4">{{ $drive->model }}</td>
                        <td class="px-6 py-4">{{ $drive->type }}</td>
                        <td class="px-6 py-4">{{ $drive->capacity }}</td>
                        <td class="px-6 py-4">{{ $drive->speed }}</td>
                        <td class="px-6 py-4">{{ $drive->condition }}</td>
                        <td class="px-6 py-4">{{ $drive->status }}</td>
                        <td class="px-6 py-4">{{ $drive->warranty }}</td>

                        <td class="px-6 py-4 text-right space-x-3">
                            <button wire:click="edit({{ $drive->id }})"
                                class="text-blue-500 text-sm font-medium hover:underline cursor-pointer">
                                Edit
                            </button>
                            <button wire:click="delete({{ $drive->id }})"
    ...
    <!-- Drive Form Modal -->
    @livewire('components.drive.form')
</div>
