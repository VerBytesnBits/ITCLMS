<div class="space-y-6">
    ...
    <div class="flex justify-center gap-6 border-b border-gray-200 dark:border-zinc-700 pb-2">
        <button wire:click="$set('tab','peripherals')" class="px-4 py-1 text-sm font-medium 
            {{ $tab === 'peripherals' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-blue-500' }}">
            Peripherals
        </button>

        <button wire:click="$set('tab','components')" class="px-4 py-1 text-sm font-medium
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-3 shadow-sm">
            ...
            <button wire:click="$set('selectedType','{{ $type }}')" class="{{ $classes }}">
                <div class="flex items-center gap-3">
                    ...
                    {{-- Icon --}}
                    <img src="{{ asset($partIcons[$type] ?? 'images/icons/default.png') }}" class="w-7 h-7 opacity-70">
        ...
        <!-- RIGHT: DEVICE LIST -->
        <div class="md:col-span-3 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm">
            ...
            <!-- ASSIGNED CARD -->
            @if ($assigned)
            <div class="bg-green-50 dark:bg-green-900/30 border border-green-300 rounded-lg p-3 mb-4 text-sm">
                <div class="flex justify-between items-center">
                    <span class="font-medium text-gray-700 dark:text-gray-200">
                        {{ $assigned->brand }} {{ $assigned->model }}
                        ({{ $assigned->serial_number }})
                    </span>
                    <button wire:click="unassign('{{ $selectedType }}')" class="px-3 py-1 rounded-md text-xs text-white bg-red-500">Unassign</button>
                </div>
            </div>
            @endif
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                <flux:button wire:click="addInlineForm('peripheral', '{{ $selectedType }}')" variant="primary" color="green">
                    ...
                    <span class="absolute -top-2 -right-2 bg-green-600 text-white text-[10px] px-2 py-[2px] rounded-full">
                        Added
                    </span>
                    @endif
                </flux:button>
            ...
            <!-- SEARCH -->
            <flux:input wire:model.live.debounce.500ms="searchPeripherals" placeholder="Search components..." autofocus />

            <div class="mt-3 space-y-2">
                @if ($available && $available->count())
                @foreach ($available as $peripheral)
                <div class="flex justify-between items-center bg-gray-50 dark:bg-zinc-800 p-3 rounded-lg text-sm border">
                    <span>
                        {{ $peripheral->brand }} {{ $peripheral->model }}
                        <span class="text-gray-500 dark:text-gray-400">(S/N:
                            {{ $peripheral->serial_number }})</span>
                    </span>
                    <button wire:click="assignSelected('{{ $selectedType }}', {{ $peripheral->id }})" @if ($mode==='create' ) disabled @endif class="px-3 py-1 text-xs rounded-md
        ...
        <div class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-3 shadow-sm">
            ...
            <button wire:click="$set('selectedPart','{{ $part }}')" class="{{ $classes }}">
                <div class="flex items-center gap-3">
                    ...
                    {{-- Icon --}}
                    <img src="{{ asset($partIcons[$part] ?? 'images/icons/default.png') }}" class="w-7 h-7 opacity-70">
        ...
        <div class="md:col-span-3 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm">
            ...
            <div class="bg-green-50 dark:bg-green-900/30 border border-green-300 rounded-lg p-3 mb-4 text-sm">
                <div class="flex justify-between items-center">
                    ...
                    <button wire:click="unassignComponent('{{ $selectedPart }}')" class="px-3 py-1 rounded-md text-xs text-white bg-red-500">Unassign</button>
                </div>
            </div>
            @endif
            ...
            <flux:input wire:model.live.debounce.500ms="searchComponents" placeholder="Search components..." autofocus />
            <!-- LIST -->
            <div class="mt-3 space-y-2">
                @if ($available && $available->count())
                ...
                <div class="flex justify-between items-center bg-gray-50 dark:bg-zinc-800 p-3 rounded-lg text-sm border">
                    ...
                    <button wire:click="assignComponent('{{ $selectedPart }}', {{ $component->id }})" @if ($mode==='create' ) disabled @endif class="px-3 py-1 text-xs rounded-md
                                         {{ $mode === 'create' ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700' }}">
                        Assign
                    </button>
    ...
    @endif
    <div x-data="{ open: @entangle('showInlineForm') }" x-show="open" x-transition:enter="transition duration-500 ease-out" class="fixed inset-0 z-50 flex justify-end" style="display: none;">



        <div x-transition:enter="transform transition duration-500 ease-out" x-transition:enter-start="translate-x-full scale-95" x-transition:enter-end="translate-x-0 scale-100" x-transition:leave="transform transition duration-300 ease-in" x-transition:leave-start="translate-x-0 scale-100" x-transition:leave-end="translate-x-full scale-95" class="relative w-full max-w-md h-full bg-white dark:bg-zinc-900 shadow-xl rounded-l-2xl p-6 overflow-y-auto flex flex-col">
            ...
            <div class="flex items-center justify-between mb-6 border-b border-gray-100 dark:border-zinc-800 pb-4">
                ...
                <button @click="open = false" class="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition duration-150 rounded-full hover:bg-gray-100 dark:hover:bg-zinc-800">
            ...
            <div class="space-y-4 flex-1 overflow-y-auto pr-2 -mr-2">
                ...
                <flux:input wire:model.defer="inline_brand" placeholder="Brand" />

                {{-- Model --}}
                <flux:label class="text-lg ">
                    <h1>Model <span class="text-red-500 text-2xl">*</span></h1>
                </flux:label>
                <flux:input wire:model.defer="inline_model" placeholder="Model" />

                {{-- Serial Number --}}
                <flux:label class="text-lg ">
                    <h1>Serial Number <span class="text-red-500 text-2xl">*</span></h1>
                </flux:label>
                <flux:input wire:model.defer="inline_serial_number" placeholder="Serial Number" />
                ...
                <flux:label class="text-lg ">
                    <h1>Capacity (GB) <span class="text-red-500 text-2xl">*</span></h1>
                </flux:label>
                <flux:input wire:model.defer="inline_capacity" placeholder="Capacity (GB)" type="number" />
                @endif
                ...
                <flux:input wire:model.defe="inline_clock_speed" mask="9.9GHz" />
                @endif
                ...
                {{-- Connection Type --}}
                <flux:input wire:model.defer="inline_connection_type" label="Connection Type" placeholder="Connection Type" />
                @endif
            ...
            <div class="mt-8 pt-4 border-t border-gray-100 dark:border-zinc-800 flex gap-3">
                <button wire:click="saveInlineItem" class="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg text-base shadow-md transition duration-200 transform hover:scale-[1.02]">
                    Save Item
                </button>

                <button @click="open = false" class="flex-1 px-4 py-2 bg-gray-200 dark:bg-zinc-700 text-gray-700 dark:text-gray-200 font-medium rounded-lg text-base hover:bg-gray-300 dark:hover:bg-zinc-600 transition duration-200">
                    Cancel
                </button>
            </div>
