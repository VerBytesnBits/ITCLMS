<div class="space-y-6">
    <div class="flex justify-between max-sm:flex-col">
        <div>
            <flux:heading size="xl" level="1" class="text-2xl! font-bold! text-zinc-500 dark:text-zinc-50 flex items-center gap-2 leading-tight italic"> Roles/Permission</flux:heading>
            <flux:subheading size="lg" class="italic">Manage all roles/permission</flux:subheading>
        </div>
    ...
    <div class="overflow-x-auto bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl shadow">
                ...
                <tr class="border-t border-gray-200 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                    <td class="px-6 py-4 font-medium text-zinc-800 dark:text-white">
                       ...
                       {{ match ($role->name) {
                        ...
                        {{-- <div class="text-xs text-gray-400">#{{ $role->id }}
    </div> --}}
    </td>
        ...
        <div class="flex flex-wrap gap-1">
            <template x-if="!showAll">
                <div class="flex flex-wrap gap-1">
                    ...
                    <span class="px-2 py-1 text-xs bg-lime-100 text-lime-800 dark:bg-lime-900 dark:text-lime-200 rounded-full">
                        {{ $permission->name }}
                    </span>
                    @endforeach

                    @if ($role->permissions->count() > 5)
                    <button @click="showAll = true" class="px-2 py-1 text-xs bg-gray-200 text-gray-600 dark:bg-zinc-700 dark:text-gray-300 rounded-full hover:underline cursor-pointer">
            ...
            <template x-if="showAll">
                <div class="flex flex-wrap gap-1">
                    ...
                    <button @click="showAll = false" class="px-2 py-1 text-xs text-blue-600 dark:text-blue-400 hover:underline ml-2 cursor-pointer">
                        Show Less
                    </button>
                </div>
            </template>
