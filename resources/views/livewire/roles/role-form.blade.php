        ...
        <div class="bg-white dark:bg-zinc-800 p-6 rounded-2xl shadow-2xl w-full max-w-xl 
            ...
            <form wire:submit.prevent="save" class="space-y-5">
                ...
                <div>
                    ...
                    <flux:input wire:model="roleName" id="roleName" placeholder="e.g. Lab Technician" :readonly="in_array($roleName, ['chairman', 'lab_technician', 'lab_incharge'])" />
                    @error('roleName')
                ...
                <div class="max-h-[60vh] overflow-y-auto pr-2 space-y-6 mt-1">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                            ...
                            <flux:checkbox label="{{ str_replace($group . '.', '', $permission->name) }}" value="{{ $permission->name }}" wire:model="permissions" class="w-full" />
                            @endforeach
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="flex justify-end pt-4 space-x-2">
                    <button type="button" wire:click="$dispatch('closeModal')" class="px-4 py-2 rounded-lg bg-gray-300 text-zinc-800 hover:bg-gray-400 dark:bg-zinc-700 dark:text-white dark:hover:bg-zinc-600 cursor-pointer">
                        Cancel
                    </button>
                    <button type="submit" class="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition cursor-pointer">
                        Save
                    </button>
                </div>
            </form>
